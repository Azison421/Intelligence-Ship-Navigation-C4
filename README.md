...
> 规划地图已改为 0.2 米水域栅格，并使用 sidecar 精确圆形浮标；
> 运行时仅在船体中心进入原始目标 0.5 米圆域后切点。当前尚未生成
> `live_ready` 新模型：第 10 个目标在 0.4 米船体半径、0.2 米硬余量和
> 非负油门五动作协议下是单入口袋，13 点连续 kinodynamic 验收未通过。
> 在动作空间或实船原地转向能力校准完成前，不应启动新 checkpoint 实跑。
